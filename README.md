...
활동지는 글의 언어에 따라 줄 방식이 달라집니다 (한글 = 원고지, 영어 = 영어 노트).

![](docs/sample-worksheet-english.png)
...
|---|---|---|
| 한글 줄 | 원고지 네모 칸 | 8자 |
| 영어 줄 | 영어 노트처럼 가로줄 네 선 | 16자 |

영어를 네모 칸에 한 자씩 넣으면 필기 연습으로 부자연스럽고, 반대로 줄만 그으면
글자 자리를 알 수 없어 인식이 어긋납니다. 그래서 **줄은 가로로 이어 긋고 안내 글자는
정해진 자리에 인쇄**합니다 — 따라 쓰는 방식이라 안내 글자가 곧 정렬 기준이 됩니다.
낱말이 잘리지 않도록 영어 줄은 낱말 단위로 넘어갑니다.

#### 무엇이 폰트가 되는가

한글 칸과 **영어 줄의 알파벳**을 걷습니다(영어 줄에는 베이스라인이 있으므로).
다만 같은 글자를 기본 영문 시트에서도 걷는데 그쪽 칸이 훨씬 커서 보통 기본 시트가
선택되고, 활동지는 기본 시트에서 빠뜨린 글자를 메워 줍니다.
아래쪽 **생각 쓰기 칸은 폰트로 만들어지지 않습니다.**

화면 오른쪽에서 **완성도**를 바로 확인할 수 있습니다. 지금 고른 조합이
프로그램에 실린 글에 나오는 글자 가운데 몇 %를 손글씨로 바꾸는지 보여 주고,
**다음에 무엇을 더하면 가장 많이 오르는지** 세 가지를 권합니다.
이미 덮은 글자는 빼고 새로 늘어나는 몫만 세므로, 고를수록 추천이 바뀝니다.

각 활동지의 **미리보기** 를 누르면 실제 인쇄 모습을 쪽 단위로 볼 수 있습니다.

```bash
node tools/coverage.js    # 활동지가 어떤 글자를 모으는지 명령줄에서 점검
```
